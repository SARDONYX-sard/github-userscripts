...
/*
 * This code is forked by SARDONYX(for non dependencies libs => speed up)
 *
 * The MIT License (MIT)
...
// @downloadURL https://raw.githubusercontent.com/SARDONYX-sard/github-userscripts/main/fork/github-in-vscode.user.js
...
(async () => {
  ...
  function unshowIconWhenPrint() {
    const css = document.createElement("style");
    css.innerHTML = "@media print { #github-in-vscode { display: none; } }";
  ...
  function setVSCodeLink() {
    ...
    link.classList.add("ghiv-link", "btn", margin, "tooltipped", "tooltipped-n");
    link.innerHTML = vsCodeIcon;

    link.setAttribute("aria-label", "Open it with github.dev");
    link.setAttribute("href", `https://github.dev${window.location.pathname}`);
    link.setAttribute("id", "github-in-vscode");
  ...
  new MutationObserver(setVSCodeLink).observe(document, { childList: true, subtree: true });
})();
